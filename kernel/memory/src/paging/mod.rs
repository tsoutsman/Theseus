...
use kernel_config::memory::{RECURSIVE_P4_INDEX, PAGE_SIZE, TEMPORARY_PAGE_VIRT_ADDR};
...
/// A page table's status.
pub trait Status {
    fn virtual_address(&self) -> *const Table<Level4>;
}
...
pub struct Inactive {
   temporary_mapping: MappedPages, 
}
impl Drop for Inactive {
    fn drop(&mut self) {
        let mapped_pages = core::mem::replace(&mut self.temporary_mapping, MappedPages::empty());
        if let Ok((_, frames)) = mapped_pages.unmap_into_parts(&mut Mapper::from_current()) {
            core::mem::forget(frames);
        } else {
            log::error!("error unmapping temporary page table mapping");
        }
    }
}
...
impl Status for Inactive {
    fn virtual_address(&self) -> *const Table<Level4> {
        self.temporary_mapping.as_type::<Table<Level4>>(0).unwrap() as *const _
    }
}
...
impl<T> fmt::Debug for PageTable<T>
where
    T: Status
{
...
where
    T: Status
{
    type Target = Mapper<T>;

    fn deref(&self) -> &Mapper<T> {
        &self.mapper
    }
}
...
where
    T: Status
{
    fn deref_mut(&mut self) -> &mut Mapper<T> {
        &mut self.mapper
    }
}
...
impl PageTable<Active> {
    ...
    pub fn switch(mut self, new_table: PageTable<Inactive>) -> Self {
        ...
        Self {
            mapper: Mapper::from_current(),
            p4_table: new_table.p4_table,
        }
    }
}

impl PageTable<Inactive> {
    pub fn new(frame: AllocatedFrames) -> Result<Self, &'static str> {
        let mut page = None;
        ...
        log::info!("allocating temp page: {:0x?}", page.as_ref().unwrap().start());
        let (mapped_page, frame) = Mapper::from_current().internal_map_to(
        ...
        Ok(Self {
            mapper: Mapper {
                status: Inactive {
                    temporary_mapping: mapped_page,
                },
                target_p4: *frame.start(),
            },
            p4_table: frame,
        })
    }
    ...
    ), &'static str>
{
    ...
    let mut new_table = PageTable::new(new_p4_frame)?;
    ...
    let new_table = page_table.switch(new_table); 
