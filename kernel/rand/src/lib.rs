...
/// Initialise the global CSPRNG.
///
/// Only the first call will initialised the CSPRNG. Subsequent calls will
...
///
/// The TSC isn't a high quality source of randomness and it is only used as a
/// last resort if both RDSEED and RDRAND failed.
fn tsc_seed() -> [u8; 32] {
    let mut seed = [0; 32];
