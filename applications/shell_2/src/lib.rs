...
//! Commands that can be run are the names of the crates in the applications
//! directory
//!
//! The shell has the following responsibilities: handles key events delivered
//! from terminal, manages terminal display, spawns and manages tasks, and
...
/// This structure is used by shell to track its spawned applications. Each
/// successfully evaluated command line will create a `Job`. Each job contains
/// one or more tasks. Tasks are stored in `tasks` in the same sequence as in
/// the command line. When pipe is used, the i-th job's `stdout` is directed to
/// the (i+1)-th job's `stdin`. `stderr` is always read by shell and currently
/// cannot be redirected.
struct Job {
    /// References to the tasks that form this job. They are stored in the same
    /// sequence as in the command line.
    tasks: Vec<JoinableTaskRef>,
    /// A copy of the task ids. Mainly for performance optimization. Task ids
    /// are stored in the same sequence as in `tasks`.
    task_ids: Vec<usize>,
    /// Status of the job.
    status: JobStatus,
    /// The stdio queues between the running application and the shell, or
    /// between running applications if pipe is used. Assume there are N
    /// tasks, counting from 0 to (N-1). `pipe_queues[0]` is the input for
    /// the 0-th task. `pipe_queues[N]` is the output from the last task to
    /// shell. Other queues are the pipes between two applications. Note
    /// that there are (N+1) queues in total.
    pipe_queues: Vec<Stdio>,
    /// The stderr queues between the running applications and the shell. Assume
    /// there are N tasks, counting from 0 to (N-1). All of these queues are
    /// sending byte streams from the applications to the shell, and
    /// currently cannot be redirected. Note that there are N queues in
    /// total.
...
/// A main function that spawns a new shell and waits for the shell loop to exit
/// before returning an exit value
pub fn main(_args: Vec<String>) -> isize {
    {
        let _task_ref =
            match spawn::new_task_builder(shell_loop, ()).name("shell_loop".to_string()).spawn() {
    ...
    // TODO: when `join` puts this task to sleep instead of spinning, we can
    // re-enable it. Otherwise, right now it kills performance.
...
enum AppErr {
    ...
    NotFound(String),
    /// The terminal could not find the application namespace due to a
    /// filesystem error.
    NamespaceErr,
...
struct Shell {
    /// Variable that stores the task id of any application manually spawned
    /// from the terminal
    ...
    cmdline: String,
    /// This buffer stores characters before sending them to running application
    /// on `enter` key strike
    input_buffer: String,
    /// Vector that stores the history of commands that the user has entered
    command_history: Vec<String>,
    /// Variable used to track the net number of times the user has pressed
    /// up/down to cycle through the commands ex. if the user has pressed up
    /// twice and down once, then command shift = # ups - # downs = 1 (cannot be
    /// negative)
    history_index: usize,
    /// When someone enters some commands, but before pressing `enter` it
    /// presses `up` to see previous commands, we must push it to
    /// command_history. We don't want to push it twice.
...
impl Shell {
    /// Create a new shell. Currently the shell will bind to the default
    /// terminal instance provided by the `app_io` crate.
    fn new() -> Result<Shell, &'static str> {
        // Initialize a dfqueue for the terminal object to handle printing from
        // applications. Note that this is only to support legacy output. Newly
        // developed applications should turn to use `stdio` provided by the
        // `stdio` crate together with the support of `app_io`.
    ...
    /// The position to insert is determined by the position of the cursor in
    /// the terminal. `sync_terminal` indicates whether the terminal screen
    /// will be synchronically updated.
    fn insert_char_to_cmdline(&mut self, c: char, sync_terminal: bool) -> Result<(), &'static str> {
        ...
        if sync_terminal {
            // disable cursor before updating in case the cursor is not at the end and the
            // old text is the prefix of the new one
    ...
    /// Remove a character from the command line buffer in the shell. If there
    /// is nothing to be removed, it does nothing and returns.
    /// The position to remove is determined by the position of the cursor in
    /// the terminal. `sync_terminal` indicates whether the terminal screen
    /// will be synchronically updated.
    ...
    /// `sync_terminal` indicates whether the terminal screen will be
    /// synchronically updated.
    ...
    /// Move the cursor a character left. If the cursor is already at the
    /// beginning of the command line, it simply returns.
    ...
    /// Move the cursor a character to the right. If the cursor is already at
    /// the end of the command line, it simply returns.
    ...
    /// Update the position of cursor. `offset_from_end` specifies the position
    /// relative to the end of the text in number of characters.
    ...
    /// Roll to the next previous command. If there is no more previous command,
    /// it does nothing.
    ...
    /// Roll to the next recent command. If it is already the most recent
    ...
    fn handle_key_event(&mut self, keyevent: KeyEvent) -> Result<(), &'static str> {
        ...
        if keyevent.modifiers.is_control() && keyevent.keycode == Keycode::C {
            let fg_job_num = if let Some(fg_job_num) = self.fg_job_num {
                ...
                // If there is no running foreground job, simply print "^C", refresh, and
                // return.
            ...
            if let Some(task_refs) = self.jobs.get(&fg_job_num).map(|job| &job.tasks) {
                ...
                app_io::lock_and_execute(&|_flags_guard, _streams_guard| {
                    ...
                    for task_ref in task_refs {
                        ...
                        // Here we must wait for the running application to quit before releasing
                        // the lock, because the previous `kill` method will
                        // NOT stop the application immediately.
                        // We must circumvent the situation where the application is killed while
                        // holding the lock. We wait for the application to
                        // finish its last time slice. It will then be
            ...
            if let Some(task_refs) = self.jobs.get(&fg_job_num).map(|job| &job.tasks) {
                ...
                app_io::lock_and_execute(&|_flags_guard, _streams_guard| {
                    ...
                    for task_ref in task_refs {
                        ...
                        // Here we must wait for the running application to stop before releasing
                        // the lock, because the previous `block` method
                        // will NOT stop the application immediately.
                        // We must circumvent the situation where the application is stopped while
                        // holding the lock. We wait for the application to
                        // finish its last time slice. It will then be truly
                        // blocked. We can thereafter release the lock.
                        loop {
                            scheduler::schedule(); // yield the CPU
                            if !task_ref.is_running() {
                                break;
        ...
        // Attempts to run the command whenever the user presses enter and updates the
            ...
            && keyevent.keycode.to_ascii(keyevent.modifiers).is_some()
        {
            ...
            if cmdline.is_empty() && self.fg_job_num.is_none() {
                // reprints the prompt on the next line if the user presses enter and hasn't
                // typed anything into the prompt
        ...
        // Adjusts the cursor tracking variables when the user presses the left and
        // right arrow keys
        ...
        // Tracks what the user has typed so far, excluding any keypresses by the
        // backspace and Enter key, which are special and are handled directly below
    ...
    /// Create a single task. `cmd` is the name of the application. `args` are
    /// the provided arguments. It returns a task reference on success.
    fn create_single_task(
        ...
        // Gets the task id so we can reference this task if we need to kill it with
        // Ctrl+C
        Ok(taskref)
    }

    /// Evaluate the command line. It creates a sequence of jobs, which forms a
    /// chain of applications that pipe the output from one to the next, and
    /// finally back to the shell. If any task fails to start up,
    /// all tasks that have already been spawned will be killed immeidately
    ...
    fn eval_cmdline(&mut self) -> Result<Vec<JoinableTaskRef>, AppErr> {
        ...
        for single_task_cmd in cmdline.split('|') {
            let mut args: Vec<String> =
                single_task_cmd.split_whitespace().map(|s| s.to_string()).collect();
    ...
    /// Try to match the incomplete command against all internal commands.
    ...
    /// Try to match the incomplete command against all applications in the same
    /// namespace. Returns a vector that contains all matching results.
    ...
    /// Try to match the incomplete command against all possible path names. For
    /// example, if the current command is `foo/bar/examp`, it first tries
    /// to walk the directory of `foo/bar`. If it succeeds, it then lists
    /// all filenames under `foo/bar` and tries to match `examp` against
    ...
    fn aligned_print_match(&mut self, possible_names: Vec<String>) -> Result<(), &'static str> {
        ...
        // If the longest string is very very long which exceeds a single line, we
        // should print them line by line. Otherwise, we put multiple results in
        // one line and separate them by four or more spaces.
    ...
    /// If there exists only one possibility, the half-entered command line is
    /// completed. If there are several possibilities, it will show all
    /// possibilities. Otherwise, it does nothing. It tries to match against
    /// all internal commands, all applications in the namespace, and all
    /// valid file paths.
    ...
    fn task_handler(&mut self) -> Result<(bool, bool), &'static str> {
        ...
        // Iterate through all jobs. If any job has exited, remove its stdio queues and
        // remove it from the job list. If any job has just stopped, mark it as
        // stopped in the job list.
        for (job_num, job) in self.jobs.iter_mut() {
                    ...
                    if let Some(exit_val) = task_ref.take_exit_value() {
                        match exit_val {
                            ExitValue::Completed(exit_status) => {
                                ...
                                // value. we know the return type of
                                // this task is `isize`, so we need
                                // to downcast it from Any to isize.
                            ...
                            ExitValue::Killed(KillReason::Requested) => {
                                // Nothing to do. We have already print "^C"
                                // while handling keyboard event.
                            }
                            ...
                            (pipe_queue_iter.next(), stderr_queue_iter.next(), task_id_iter.next())
                        {
                            ...
                            if *task_id == exited_task_id {
                                ...
                                // reader of the queue that the
                                // stream has ended. The `if let` clause should not
                                // fail.
            ...
            // If the job is stopped (e.g. by ctrl-Z), print an notification to the
            // terminal.
            #[cfg(not(bm_ipc))]
            {
                if is_stopped {
            ...
            // Record the completed job and remove them from job list later if all tasks in
            // the job have exited.
            if !has_alive {
        ...
        // Actually remove the exited jobs from the job list. We could not do it
        // previously since we were iterating through the job list. At the same
        // time, remove them from the task_to_job mapping, and remove the queues
        // in app_io.
    ...
    /// If there is any output event from running application, print it to the
    ...
    fn check_and_print_app_output(&mut self) -> bool {
        ...
        if let Some(print_event) = self.print_consumer.peek() {
            ...
            // Goes to the next iteration of the loop after processing print event to ensure
            // that printing is handled before keypresses
            need_refresh = true;
        }
    ...
    /// This main loop is the core component of the shell's event-driven
    /// architecture. The shell receives events from two queues
    ///
    /// 1) The print queue handles print events from applications. The producer
    /// to this queue    is any EXTERNAL application that prints to the
    /// terminal.
    ///
    /// 2) The input queue (provided by the window manager when the temrinal
    /// request a window) gives key events    and resize event to the
    /// application.
    ///
    /// The print queue is handled first inside the loop iteration, which means
    /// that all print events in the print queue will always be printed to
    /// the text display before input events or any other managerial functions
    /// are handled. This allows for clean appending to the scrollback
    /// buffer and prevents interleaving of text.
    fn start(mut self) -> Result<(), &'static str> {
        ...
        loop {
            // If there is anything from running applications to be printed, it printed on
            // the screen and then return true, so that the loop continues,
            // otherwise nothing happens and we keep on going with the
            ...
            // returns whether we need a new prompt or need to refresh the
            // screen.
            ...
            while let Some(ev) = {
                // this weird syntax ensures the terminal lock is dropped before entering the
                // loop body
                let mut locked_terminal = self.terminal.lock();
                ...
                match ev {
                    ...
                    Event::WindowResizeEvent(new_position) => {
                        ...
                        // the above function also refreshes the terminal
                        // display
                    }
                    ...
                    _unhandled => {
                        // trace!("Shell is ignoring unhandled event: {:?}",
                        // _unhandled);
                    }
                };
            }
...
impl Shell {
    ...
    /// Execute the command line as an internal command. If the current command
    /// line fails to be a shell internal command, this function does
    /// nothing.
    ...
    /// Execute `bg` command. It takes a job number and runs the in the
    /// background.
    ...
    /// Execute `fg` command. It takes a job number and runs the job in the
    /// foreground.
...
/// Start a new shell. Shell::start() is an infinite loop, so normally we do not
